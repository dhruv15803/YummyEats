...
const registerRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    const cuisines = restaurantCuisines.map((c: any) => c._id);
...
const getMenuItems = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const menuItems = await MenuItem.find({ restaurant_id: id }).populate(
      "item_cuisine"
    );
    res.status(200).json({
      success: true,
...
const addMenuItem = async (req: Request, res: Response) => {
  try {
    ...
    const newMenuItem = await MenuItem.findOne({ _id: menuItem._id }).populate(
      "item_cuisine"
    );
    res.status(200).json({
      success: true,
...
const editMenuItem = async (req: Request, res: Response) => {
  try {
    const {
      newMenuItemName,
    ...
    const updatedItem = await MenuItem.findOne({ _id: menuItem._id }).populate(
      "item_cuisine"
    );
    res.status(200).json({
      success: true,

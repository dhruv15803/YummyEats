...
const AdminCuisine = () => {
    ...
    const addCuisine = async (e: React.FormEvent<HTMLFormElement>) => {
        try {
            ...
            const response = await axios.post(`${backendUrl}/api/admin/addCuisine`,{
                cuisineName,
    ...
    const deleteCuisine = async (id:string) => {
        try {
            const response = await axios.delete(`${backendUrl}/api/admin/deleteCuisine/${id}`,{
                withCredentials:true,
            })
            console.log(response);
    ...
    <div className='flex flex-col p-4 border rounded-lg shadow-md mx-10'>
        <form onSubmit={(e) => addCuisine(e)} className='flex items-center gap-2 mb-4'>
            <Input value={cuisineName} onChange={(e) => setCuisineName(e.target.value)} type='text' name='cuisineName' id='cuisineName' placeholder='eg: indian'/>
            <Button>Add cuisine</Button>
        </form>
        {addCuisineErrorMsg!=="" && <div className='text-red-500'>{addCuisineErrorMsg}</div>}
        <div className='flex flex-col gap-2'>
            {cuisines.length!==0 ? <>
                {cuisines.map((cuisine) => {
                    return <AdminCuisineCard key={cuisine._id} cuisine={cuisine} deleteCuisine={deleteCuisine}/>
                })}
            </> : <div className='text-gray-500'>Website has no supported cuisine options for restaurants</div>}
        </div>
    </div>
    </>
  )
